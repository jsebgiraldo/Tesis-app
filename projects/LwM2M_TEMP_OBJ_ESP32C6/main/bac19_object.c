...
static int bac_read(anjay_t *anjay, const anjay_dm_object_def_t *const *def,
    ...
    switch (rid) {
    case RID_DATA: {
        if (e->data.ptr && e->data.size > 0) {
            return anjay_ret_bytes(ctx, e->data.ptr, e->data.size);
        }
...
static int bac_write(anjay_t *anjay, const anjay_dm_object_def_t *const *def,
        ...
        do {
            ...
            if (bytes_read > 0) {
                ...
                if (e->data.size + bytes_read > e->data.capacity) {
                    ...
                    while (new_cap < e->data.size + bytes_read) {
                        new_cap *= 2;
                    }
    ...
    case RID_DATA_CT: {
        int64_t v = 0;
        int res = anjay_get_i64(ctx, &v);
        if (!res) e->ctime = v;
        return res;
    }
    case RID_DATA_DESC: {
        char tmp[128];
        int res = anjay_get_string(ctx, tmp, sizeof(tmp));
        if (!res) {
            free(e->desc);
            e->desc = strdup(tmp);
        }
